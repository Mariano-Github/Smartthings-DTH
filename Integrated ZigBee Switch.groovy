 ...
 */
public static String version() { return "v0.0.27c.20210315_Lidl_Mod" }
/*
 *   2021/03/15 >>> v0.0.27c_Lid Mod - Mod healtPoll(), zigbee.onOffConfig() and device check intervals (By MCC, from christianH21220)
...
def healthPoll() {
    log.trace "healthPoll()"
    def cmds = refresh()
    cmds.each { sendHubCommand(new physicalgraph.device.HubAction(it)) }
}

def configureHealthCheck() {
    log.trace "configureHealthCheck()"
    Integer hcIntervalMinutes = 62 //modified from value 12 (mar/2021 MCC)
    if (!state.hasConfiguredHealthCheck) {
        log.debug "Configuring Health Check, Reporting"
        unschedule("healthPoll")
        runEvery30Minutes("healthPoll") //modified from runEvery5Minutes (mar/2021 MCC)
...
def configure() {
    ...
    def cmds = zigbee.onOffConfig(0, 660) //modified 660 from 120 (mar/2021 MCC)
    def endpointCount = getEndpointCount()
    ...
    if (endpointCount > 1) {
        childDevices.each {
            ...
            if (childEndpoint.isNumber()) {
                ...
                def endpointInt = zigbee.convertHexToInt(childEndpoint)
                cmds += zigbee.configureReporting(zigbee.ONOFF_CLUSTER, 0x0000, 0x10, 0, 660, null, [destEndpoint: endpointInt])  //modified 660 from 120 (mar/2021 MCC)
            }
        }
    } else {
        cmds += zigbee.configureReporting(zigbee.ONOFF_CLUSTER, 0x0000, 0x10, 0, 660, null, [destEndpoint: 0xFF])  //modified 660 from 120 (mar/2021 MCC)
    }
    cmds += refresh()
    return cmds
}
