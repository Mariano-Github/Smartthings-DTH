...
metadata {
	definition(name: "SmartSense Motion Sensor-Greg", namespace: "smartthings", author: "SmartThings", runLocally: true, minHubCoreVersion: '000.017.0012', executeCommandsLocally: false, mnmn: "SmartThings", vid: "generic-motion", genericHandler: "Zigbee") {
...
def configure() {
	...
	if (isFrientSensor()) {
		configCmds += zigbee.configureReporting(zigbee.TEMPERATURE_MEASUREMENT_CLUSTER, 0x0000, DataType.INT16, 30, 43200, 300, [destEndpoint: 0x26])
	} else {
		configCmds += zigbee.temperatureConfig(30, 43200, 300)
	}
